...
telemetry_expired_topic = config(
    "KAFKA_TOPIC_TELEMETRY_EXPIRED", default="telemetry.expired"
)
...
def get_telemetry_raw_producer() -> KafkaProducer:
    return KafkaProducer(
        config=ProducerConfig(), topic=telemetry_raw_topic, poll_timeout=0.01
    )
...
def get_telemetry_clean_producer() -> KafkaProducer:
    return KafkaProducer(
        config=ProducerConfig(), topic=telemetry_clean_topic, poll_timeout=0.01
    )
...
def get_telemetry_dlq_producer() -> KafkaProducer:
    return KafkaProducer(
        config=ProducerConfig(), topic=telemetry_dlq_topic, poll_timeout=0.01
    )
...
def get_telemetry_expired_producer() -> KafkaProducer:
    return KafkaProducer(
        config=ProducerConfig(), topic=telemetry_expired_topic, poll_timeout=0.01
    )
